$(function () {
    ...
    chatHub.client.addChatRoom = function (room) {
        model.roomAdded(new ChatRoom(room.Id, room.Name, room.FromUserName, room.ToUserName));
    };
    ...
    Model.prototype = {
        ...
        roomList: function () {
            ...
            chatHub.server.getRooms().done(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    self.chatRooms.push(new ChatRoom(result[i].Id, result[i].Name, result[i].FromUserName, result[i].ToUserName));
                }
            });
        },
            ...
            chatHub.server.getUsers(self.joinedRoom()).done(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    self.chatUsers.push(new ChatUser(result[i].Username,
                        result[i].FirstName + " " + result[i].CustomUserId,
                        result[i].Avatar,
        ...
        createRoom: function () {
            var name = $("#roomName option:selected").text();
            ...
            chatHub.server.getMessageHistory(self.joinedRoom()).done(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    self.chatMessages.push(new ChatMessage(result[i].Content,
                        result[i].Timestamp,
                        result[i].From,
                        result[i].ismine == "yes" ? true : false,
                        //isMine,
                        result[i].Avatar))
                }
    ...
    function ChatRoom(roomId, name, fromuser, touser) {
        var self = this;
